...
import { fetchUsers, addUser, updateUser, deleteUser, type User } from './usersSlice';
import type { RootState, AppDispatch } from '../../store';

const UsersScreen: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const { users, fetchLoading, addLoading, updateLoading, addError, updateError, deleteError } = useSelector((state: RootState) => state.users);
  ...
  useEffect(() => {
    if (prevAddLoadingRef.current && !addLoading && isModalOpen && !addError && !isEdit) {
  ...
  useEffect(() => {
    if (prevUpdateLoadingRef.current && !updateLoading && isModalOpen && !updateError && isEdit) {
      setIsModalOpen(false);
      setIsEdit(false);
      setEditingUser(null);
      form.resetFields();
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between mt-4">
        <Button type="primary" icon={<PlusOutlined />} onClick={showModal} loading={addLoading}>Add User</Button>
        <Button type="default" className="ml-2" onClick={() => navigate('/iteams')}>
          Iteams
        </Button>
      </div>
      <Modal
